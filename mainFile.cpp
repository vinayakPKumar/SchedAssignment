...
/***********************Local functions*******************************************************************/
static int simulator(uint16_t duration_u16,uint8_t numberOfTasks_u8,vector <taskInfo_st> taskDetails_ast,int totalNumOfChannels_u8,vector <channelInfo> channelDetails_ast,bool useGravity,int numberOfMergedCh);
static float calculateTOA(float SF_u8, float PL_u8);
static void simulationSets();
bool detailedDisplay =false;
/**********************Main function***********************/
 ...
 * Return:  None
**********************************************************/
static int simulator(int duration_u16,uint8_t numberOfTasks_u8,vector <taskInfo_st> taskDetails_ast,int totalNumOfChannels_u8,vector <channelInfo> channelDetails_ast,bool useGravity,int numberOfMergedChannels)
{
                ...
                if(transmittingList[i].remainingTxTime == 0)
                {
                    ...
                    for(int j = 0; j<totalNumOfChannels_u8;j++)
                    {
                        if(transmittingList[i].currentChannel == channelDetails_ast[j].channelID_u8)  //Search for the channel which is used
                        {
                            ...
                            if(channelDetails_ast[j].availability_bo == false)  //If a channel was blocked due to multiple links transmitting on it, unblock it since one of the links has now stopped.
      ...
      for(int l = 0;l<totalNumOfChannels_u8;l++)
            {
                ...
                if(channelDetails_ast[l].numberOfLinksOngoing > (numberOfMergedChannels-1))
...
static void simulationSets()
{
    int numberOfChannels, numberOfNodes,numberOfSimulations,simulationTime,numberOfMergedChannels,deadlineLower,deadLineUpper;
    ...
    cout<<"Enter the number of transmissions you want to have simulataneously on one channel(Between 1 and 6)"<<endl;
    cin>>numberOfMergedChannels;
    cout<<"Enter y if you want to display scheduling details"<<endl;
    cin>>enteredVal;
    if(enteredVal == 'y')
        detailedDisplay = true;
    cout<<"Enter the lower value for deadline."<<endl<<"Ex: Entering a value of 1 implies that the lowest possible deadline = 1*TimeOnAir. The value 2 implies 2*TimeOnAir and so on."<<endl;
    cin>>deadlineLower;
    cout<<"Enter the upper value for deadline."<<endl<<"Ex: Entering a value of 5 implies that the highest possible deadline = 5*TimeOnAir. The value 6 implies 6*TimeOnAir and so on."<<endl;
    cin>>deadLineUpper;
...
repeat: for(int j =0;j<numberOfNodes;j++)  //Generate TOA for a random payload for each node
        {
            ...
            randomPayload = 1 + (rand()%100);   //Assign a random payload between 1 and 100 bytes
            nodeList[j].computationTime_u16 = calculateTOA(nodeList[j].SpreadFactor,randomPayload);
            nodeList[j].deadline_u16 = nodeList[j].computationTime_u16 * (deadlineLower+(rand()%(deadLineUpper-(1+deadlineLower))));  //Deadline assigned as a random value between TOA and 5*TOA
        }
       ...
       average += simulator((int)(commonPeriod*simulationTime),numberOfNodes,nodeList,numberOfChannels,channelList,gravityOn,numberOfMergedChannels);  //Call the simulator
       if(detailedDisplay == true)
            cout<<"***********************"<<endl;
    }
